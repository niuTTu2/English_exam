...
用户将在后续提供唯一新文章，本轮未收到。正式试点：**未开始**。架构测试夹具不是考研试点，不能代替原卷逐字核对、翻译和推理的语义验收。没有试点与完整验收前不推生产 main。
...
1. **正式试点文章尚未提供**。合成夹具只验证工程路径；不替代原卷、译文、关键句选择、错因和证据的语义验收。
2. **360px/390px 实际浏览器布局及触控未通过验收**。已建立 `tests/browser-v2` 双独立宽度页面，但当前云浏览器拒绝本机地址，返回 `net::ERR_BLOCKED_BY_CLIENT`。响应式样式与SSR通过不等于无横向溢出实测。
3. **V2 完整浏览器闭环未验收**：做题→解析→原句→返回题目、词卡→原始题干/选项、手动范围标记、刷新恢复、跨账号切换与追问输入，需要在可访问的真实新文章中实际操作。
4. **V2 离线重开未实测**。沿用既有SW的已访问资源缓存，提供首次未缓存错误提示；无后端AI也无新增外部服务。
5. **本提交未部署到正式站**。生产 main 保持审计基线；独立分支提交供审阅。没有触发登录、邮箱或部署调整。

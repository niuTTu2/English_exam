...
用户后续要求直接提交，架构及2013试点已由main提交 `f368c75` 合并；正式站已经实际打开2013四页。早期审计与待发布记录保留原文，不能用它们覆盖当前发布状态；手机触控与离线验收仍单独记录。

本次将做题页标记拆成“标单词 / 标词组 / 标句子”。点击原文只产生临时预选，确认按钮才写入原有 `articleV2Marks`；独立的取消选择不写记录，取消已存标记保持 `active:false`，撤销核对目标记录版本后恢复之前状态。全文只有一个临时预选，切模式或切来源不会把两句话拼成一个词组。原有稳定键和同步协议不变。

快速读懂原句的每个英文词均通过来源ID打开现有词卡；查词不直接加入待学，也不切换难句标记。原文标点及空格保持原样，已有词/词组标记在读句中仍可见。模块位于 `source-selection.ts` 与 `source-marking.tsx`，不增加公共学习字段，不修改V1组件。详见 `docs/change-reports/v2-marking-interaction.md`。
...
新增本机完整应用入口 `tests/browser-v2/serve-pilot.mjs` 及360/390测量包装页，操作见 `docs/V2_BROWSER_ACCEPTANCE.md`。它加载真实StudyApp、现有API及SW，不修改生产应用，也不复用合成文章来验收试点。`--smoke`退出0，包装页与真实应用首页均返回200；脚本语法、ESLint和差异检查通过。

当前浏览器访问新入口仍返回 `ERR_BLOCKED_BY_CLIENT`，GitHub试点提交未返回预览部署状态，手机/离线/正式站仍未通过。新增线上预览属于AGENTS明确要求单独授权的部署范围；未创建外部服务、未推main。

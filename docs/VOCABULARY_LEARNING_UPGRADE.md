...
## 当前接续：阅读标记与原形（2026-09-20）

用户要求V2原文确认的生词直接进入共享词汇学习。`reading-marks.ts`按精确来源范围复用corpus和sense/词组记忆：本次确认与记忆同一次快照保存，旧活动标记在学习入口幂等接续，保留既有暂停、到期、历史和不同义项。只预选或取消预选不加入；确认后的撤销只撤回本次未发生后续学习的加入；普通取消原文高亮不删除学习进度，可在词汇学习中暂停。已标记范围包含准确sense的记忆，复用原学习队列。

共享词卡标题与单词学习卡显示headword，原文仍按原形变化前的expression高亮；本句词形单列。`word-forms.tsx`展示已有原形映射中的词形及已精审说明，不再把“没有特殊变形”当成“没有其他词形”。`lexicon.ts`按需反查已有alias和词形说明；不使用词族关系合并派生词，不按词尾猜原形，不改词条或记忆ID。资料按需展开，学习正面不泄露本句义。检查与限制见 `change-reports/reading-marks-word-forms.md`。
...
## 16. 同义措辞重复义项与重复学习卡修复（2026-09-20）

用户手机截图显示 `and` 的“和”“并且”“和；并且”等被拆成14行。根因是兜底归组依赖规范化后的中文释义完全相同：虽然防止了多义混淆，却把同一意思的改写、指代对象和语法角色说明误当成新义。该问题也影响记忆项、学习队列和旧记录选取例句，不能只合并界面行。

本次新增三份按现有词条稳定键、词性和完整释义审阅的别名映射，共覆盖274个词条。它们只关联已有释义到稳定的语义标识，不提供另一套词库，不修改任何真题词义、原文、分析或答案。通用规范化处理标点、重复释义词和顺序；明确的同义表达复用语义标识；未审阅的片段相似、包含关系或共同汉字不能作为合并依据。功能词按真实角色归并，例如不同宾语内容的that属于同一内容连接功能，不同指代对象的it不产生新词义；that的指示/关系/内容连接、be的系动词/被动/进行时仍分开。

查词和学习答案面的全部义项使用相同规则，按合并后的真实出处数排序。同一出处只计一次，同时保留该出处所有不同词形、词性说明、本句义和具体用法，不把每行次数简单相加。当前词卡仍显示原句的准确本句义。打开的单词按需聚合与缓存，别名索引也按词创建；首页只处理已保存记忆，不扫描全库。

旧记忆采用**只派生语义分组、不删除旧记录**的兼容方案：原memory ID、主出处、所有语境、attempt和session ID继续保留，未增加持久化字段或改变同步格式。新语境优先复用同义旧ID；已存在的重复记录每组只进入一张初始学习卡，首页、每日目标、总结和可选拼写也按组计数。组内原本到期最早的已学习记录优先，单纯查看/打开队列不改原到期时间。只有用户实际评分后，才统一同组活跃别名的阅读调度，采用保守的已掌握次数和当日晋级约束；各条旧拼写记录独立保留。暂停与恢复按义项处理。

恢复旧会话保留已完成前缀、原尝试记录与刻意安排的重现卡，仅移除剩余的重复初始卡；先去重再插入忘记项，确保有足够项目时仍隔4张真实卡重现。本句语境可在同义旧记忆之间轮换，既有历史出处不被替换。旧词条自动选取学习语境也按合并后的义项累计真实来源，继续无需用户补语境。

新增专项覆盖实际and/content/function词、规范化、同出处多种解释保留、真正不同义项与词性隔离、别名旧记录无损、逾期/未来过滤、同日不得反复升级、旧会话恢复及重现间隔、跨文章范围数量、语境轮换与拼写去重。旧内容和既有测试断言未删除或放宽。混合多个词性/意思且缺乏确切归类的旧宽泛释义继续保留，不为消除所有列表行而猜测单一含义；本轮不宣称所有中文同义改写均可自动判断。

全库复算以升级前1,054个多行词条为比较集合（多行不等于有错），升级后242个词的显示行减少，合计减少1,362行重复措辞。真实样例：and从14行到1行，共276处；new从7行到1行，共36处；become从5行到1行，共20处；problem从5行到1行，共14处；important从2行到1行，共4处。way从7行到4行，方法与道路、进展、方面继续区分。所有原语料记录仍然存在，上述减少指显示分组，不是删数据。

最终检查：`npm run quality`退出0，包含`npm run lint`（0错误/6个既有警告）、`npm run quality:content`（171/171）、`npm run build`（成功）及构建后的`node --test --test-concurrency=1 tests/*.test.mjs`（394/394，0失败、0跳过，305.7秒）。审查中的as词性边界等最后修正均早于该生产构建；之后单独`npm run lint`仍退出0。`./node_modules/.bin/tsc --noEmit --incremental false --pretty false`仍退出2，10个旧错误与基线逐字一致，无新增错误，不宣称全库类型检查通过。`git diff --check`通过；冻结内容、原词条稳定键、认证/同步/部署配置没有修改。

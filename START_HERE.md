...
用户现已授权继续完成2013整卷，替代下方历史“仅Text 1试点”的导入范围限制。Text 1及全部V1内容保持冻结；其余新阅读Text 2—4使用V2，完形、新题型、翻译、写作按真实题型使用既有体系。逐篇核对用户DOCX、测试、独立提交并通过既有main→Cloudflare发布，不再请求逐篇批准。词汇复用原形、稳定sense与词组键；同一核心意思的不同中文表述合并标签，真正异义分开，保留全部准确出处与学习记录。Text 2已发布并实测，Text 3已完成内容与回归，下一篇Text 4；实际完成情况见`docs/IMPORT_PROGRESS.md`。

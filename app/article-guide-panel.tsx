...
export function ArticleGuidePanel({ article, onSentence, onOpen }: { article: ArticleContent; onSentence: (id: string, at: number) => void; onOpen?: (at: number) => void }) {
  ...
  return <details className="article-guide" onToggle={event => { if (event.target === event.currentTarget && event.currentTarget.open) onOpen?.(Date.now()); }}><summary>文章地图 · 把句子放回全文</summary>

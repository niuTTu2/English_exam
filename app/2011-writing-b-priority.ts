...
export function writing2011BPriority(entry: Pick<VocabEntry, "headword" | "display" | "kind" | "canonicalForm">): VocabularyPriority {
  const head = entry.headword.toLowerCase();
  if (entry.kind === "phrase") return { id: "structure", label: "图表写作结构", reason: "把依据、评论或数量限定作为完整表达学习，并确认适用关系。", recommendedReview: true };
  ...
  if (["write", "essay", "interpret", "chart", "comment", "word"].includes(head)) return { id: "core", label: "图表作文常用词", reason: "连同图表解读与评论任务学习用法，再迁移到其他图表作文。", recommendedReview: true };

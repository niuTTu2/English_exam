...
export type HomeMetrics = { dueWords: number; duePhrases: number; overdue: number; completed: number; newWords: number; newPhrases: number; pendingWords: number; learnedWords: number; remainingNew: number; estimatedMinutes: number };
...
export function VocabularyHome({ metrics, settings, scope, articleLabel, year, lists, resumable, busy, message, onScope, onSettings, onStart, onResume }: VocabularyHomeProps) {
  ...
  return <section className="vl-home" aria-label="词汇学习首页">
    <header className="vl-home-heading"><span className="vl-eyebrow">在真题里记住它</span><h2>我的全局生词库</h2><p>在正文、题干或选项中标记后，直接进入待复习。所有年份共用，切换试卷不会丢失。</p></header>
    <div className="vl-primary-actions">
      ...
      <button type="button" className="vl-action vl-action-accent" disabled={!due || busy} onClick={() => onStart("review")}><strong>全局待复习 <b>{due}</b></strong><span>{due ? `单词 ${metrics.dueWords} · 词组 ${metrics.duePhrases}，每组分批完成` : "今天还没有到期项目"}</span></button>
      <button type="button" className="vl-action" disabled={busy} onClick={() => onStart("new")}><strong>学习待学单词</strong><span>{settings.reviewFirst && due ? "先处理到期项，再开始待学内容" : `今日目标 ${settings.dailyWords} 个单词 + ${settings.dailyPhrases} 个词组`}</span></button>
    </div>
    {message && <p className="vl-message" role="status">{message}</p>}
    <dl className="vl-home-metrics"><div><dt>到期单词</dt><dd>{metrics.dueWords}</dd></div><div><dt>到期词组</dt><dd>{metrics.duePhrases}</dd></div><div><dt>待学单词</dt><dd>{metrics.pendingWords}<small>尚未完成首次学习</small></dd></div><div><dt>已学单词</dt><dd>{metrics.learnedWords}<small>按首次学习日期归档</small></dd></div><div><dt>当前逾期积压</dt><dd>{metrics.overdue}</dd></div><div><dt>本组预计</dt><dd>{metrics.estimatedMinutes}<small>分钟，按每卡约半分钟</small></dd></div></dl>
    <div className="vl-scope-row"><label htmlFor="vl-learning-scope">待学筛选范围</label><select id="vl-learning-scope" value={scope.kind} onChange={event => onScope({ kind: event.target.value as LearningScope["kind"], list: scope.list ?? lists[0] })}><option value="article">当前文章 · {articleLabel}</option><option value="year">当前年份 · {year}</option><option value="all">全部年份（默认）</option><option value="marked">我标记的词</option><option value="list">自定义清单</option></select>{scope.kind === "list" && <label className="vl-list-select">选择清单<select aria-label="选择词汇清单" value={scope.list ?? lists[0] ?? ""} onChange={event => onScope({ ...scope, list: event.target.value })}>{lists.length ? lists.map(list => <option key={list} value={list}>{list}</option>) : <option value="">尚未创建清单</option>}</select></label>}</div>
    ...
    <details className="vl-plan"><summary>每日计划与学习偏好</summary><div className="vl-settings-numbers"><label>每日新单词<input type="number" inputMode="numeric" min={0} max={300} value={settings.dailyWords} onChange={event => changeNumber("dailyWords", event.target.value)} /></label><label>每日新词组<input type="number" inputMode="numeric" min={0} max={300} value={settings.dailyPhrases} onChange={event => changeNumber("dailyPhrases", event.target.value)} /></label><label>单次学习数量<input type="number" inputMode="numeric" min={1} max={300} value={settings.sessionSize} onChange={event => changeNumber("sessionSize", event.target.value)} /></label></div>
      ...
      <p>今天已完成首次学习：{metrics.newWords} 个单词、{metrics.newPhrases} 个词组。今天还可学习 {metrics.remainingNew} 项待学内容。</p>
    </details>
  </section>;
}

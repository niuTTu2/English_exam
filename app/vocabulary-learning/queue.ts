...
import { distinctMemoryKeys, memoryGroups, representativeMemory, withGroupContexts } from "./memory-groups";
...
export function nextContextId(memory: VocabularyMemory, attempts: Record<string, VocabularyAttempt> = {}, aliases = [memory.id]) {
  const ids = new Set(aliases);
  const prior = Object.values(attempts).filter(attempt => ids.has(attempt.memoryId) && attempt.kind === "reading")
...
export function createLearningQueue(
  ...
  const inScope = (memory: VocabularyMemory) => !permitted || grouping.byId.get(memory.id)?.some(member => permitted.has(member.id));
  const reviews = dueMemories(memories, now).filter(inScope);
  const newItems = grouping.groups.map(representativeMemory).filter((memory): memory is VocabularyMemory => Boolean(memory && memory.status === "unseen" && inScope(memory)))
  ...
  const learnedWords = distinctMemoryKeys([...todayNewIds].filter(id => memories[id]?.kind === "word"), memories).size;
  const learnedPhrases = distinctMemoryKeys([...todayNewIds].filter(id => memories[id]?.kind === "phrase"), memories).size;
  ...
  const selected = ordered.slice(0, size).map(memory => {
    const view = withGroupContexts(memory, grouping.byId.get(memory.id) ?? [memory]);
    return view === memory ? memory : { ...view, updatedAt: now };
  });
  const queue: VocabularyQueueItem[] = selected.map((memory, index) => ({
    ...
    contextId: nextContextId(memory, attempts, grouping.byId.get(memory.id)?.map(member => member.id)),
  }));
...
export function vocabularyTodayStats(memories: Record<string, VocabularyMemory>, attempts: Record<string, VocabularyAttempt>, now: number) {
  ...
  return { dueWords: due.filter(memory => memory.kind === "word").length,
    ...
    completed: distinctMemoryKeys(today.filter(attempt => attempt.kind === "reading").map(attempt => attempt.memoryId), memories).size,
    newWords: distinctMemoryKeys(today.filter(attempt => attempt.wasNew && memories[attempt.memoryId]?.kind === "word").map(attempt => attempt.memoryId), memories).size,
    newPhrases: distinctMemoryKeys(today.filter(attempt => attempt.wasNew && memories[attempt.memoryId]?.kind === "phrase").map(attempt => attempt.memoryId), memories).size,

...
export function QuestionEvidencePanel({ question, onSentence, openSections, onSectionToggle }: { question: AnyQuestion; onSentence: (id: string, option?: string) => void; openSections?: string[]; onSectionToggle?: (id: string, open: boolean) => void }) {
  ...
  return <section className="question-evidence-panel" aria-label={`第${question.number ?? question.id}题推理过程`}>
    ...
    <details {...detailProps("options")}><summary>4. 逐项判断与干扰方式</summary>{question.options.map(option => {
    ...
    {reasoning.wordingNotes && <details {...detailProps("wording")}><summary>难选项按需拆开读</summary>{reasoning.wordingNotes.map(note => <div key={note.sourceId}><strong>{note.text}</strong><p>{note.explanation}</p></div>)}</details>}

...
export function QuestionEvidencePanel({ question, onSentence }: { question: AnyQuestion; onSentence: (id: string) => void }) {
  ...
  return <section className="question-evidence-panel" aria-label={`第${question.number ?? question.id}题推理过程`}>
    ...
    <details><summary>2. 到哪里找证据（{reasoning.evidence.length}处）</summary>
    ...
    <details><summary>3. 原文怎样走到选项</summary>{reasoning.paraphrases.map((chain, index) => <div key={index} className="paraphrase-chain"><b>{chain.relation}</b>{chain.evidenceIds.map(id => <blockquote key={id}>{reasoning.evidence.find(e => e.id === id)?.quote}</blockquote>)}<p>→ {chain.meaning}</p><strong>→ {chain.optionText}</strong><p>{chain.limit}</p></div>)}</details>
    <details><summary>4. 逐项判断与干扰方式</summary>{question.options.map(option => {
      const judgment = reasoning.options[option.key];
      return <div key={option.key} className="option-reasoning"><h5>{option.key} · {judgment.judgment}{judgment.errorType ? ` · ${judgment.errorType}` : ""}</h5><p>{judgment.reasoning}</p><div>{judgment.evidenceIds.map(id => { const e = reasoning.evidence.find(item => item.id === id); return e && <button type="button" key={id} className="guide-source-link" onClick={() => onSentence(e.sentenceId)}>{e.role}</button>; })}</div></div>;
    })}</details>
    {reasoning.wordingNotes && <details><summary>难选项按需拆开读</summary>{reasoning.wordingNotes.map(note => <div key={note.sourceId}><strong>{note.text}</strong><p>{note.explanation}</p></div>)}</details>}

...
export function ArticleVocabularyPage({ article, data, corpus, onUpdate, onTerm, onSource }: {
  ...
  return <section aria-label="词汇搭配">
    ...
    {learning ? <VocabularyLearning onTerm={onTerm} data={data} onUpdate={update => onUpdate(current => ({ ...current, ...update(current) }))} corpus={corpus} articleId={article.id} articleLabel={article.label} year={article.year} lists={data.lists} listItems={data.listItems} marks={data.marks} notes={data.termNotes} onNote={(key, note) => onUpdate(current => ({ ...current, termNotes: { ...current.termNotes, [key]: note } }))} onSource={onSource} /> : Object.entries(vocabularyCategories).map(([category, label]) => <section key={category}><h3>{label}</h3>

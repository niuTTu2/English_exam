import { trainingSources } from "./training-sources";
...
export function TrainingReview({ articles, attempts, reflections, questionWork, locationAttempts = {}, submitted, onSentence, onQuestion, now }: {
  ...
  const tasks = articles.flatMap(article => trainingSources(article).flatMap(sentence => (sentence.practice ?? []).map(task => ({ article, sentence, task, result: latestTaskAttempt(attempts, task, sentence.id) }))));
  ...
  return <section className="training-review">
    ...
    <div className="training-review-list">{due.map(({ sentence, task, result }) => <button key={`${sentence.id}-${task.id}`} type="button" onClick={() => onSentence(sentence.id)}><strong>{sentence.number ? `第${sentence.number}句` : "文章地图"} · {grammarConcepts[task.conceptId]}</strong><span>{task.prompt}</span><small>{result?.correct ? "到期巩固" : errorCategories[task.errorType]}</small></button>)}</div>

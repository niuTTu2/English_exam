...
import { translation2011SourcePhraseGuides, translation2011SourcePhraseAliases, translation2011SourceCollocationGlosses } from "./2011-translation-collocations";
...
import { getCloze2001SourceKnowledge } from "./2001-cloze-contexts";
import { getPassage2001P2SourceKnowledge } from "./2001-passage-2-contexts";
...
import { passage2000P1QuestionPhraseGuides, passage2000P1QuestionPhraseAliases, passage2000P1QuestionCollocationGlosses } from "./2000-passage-1-question-knowledge";
import { getPassage2000P1WordKnowledge } from "./2000-passage-1-contexts";
...
import { passage2012P2PhraseGuides, passage2012P2PhraseAliases, passage2012P2PhraseGlosses, getPassage2012P2WordKnowledge } from "./2012-passage-2-knowledge";
...
import {
  passage4CollocationGlosses,
  passage4FamilyGlosses,
  passage4PhraseAliases,
  passage4PhraseGuides,
  passage4WordKnowledge,
} from "./passage-4-knowledge";
import {
  passage5CollocationGlosses,
  passage5FamilyGlosses,
  passage5PhraseAliases,
...
import {
  passage2010P1CollocationGlosses,
  passage2010P1FamilyGlosses,
  passage2010P1PhraseAliases,
...
const collocationGlosses: Record<string, { meaning: string; note?: string }> = {
  "a decrease in": { meaning: "……方面的减少 / 下降", note: "in 后接发生变化的对象。" },
  "a defining characteristic": { meaning: "决定性特征；典型特征" },
  ...
  "a serious disadvantage": { meaning: "严重的不利条件" },
  "a serious problem": { meaning: "严重的问题" },
  ...
  "be characteristic of": { meaning: "是……所特有的；具有……的典型特征", note: "of 后接具有该特征的人或事物。" },
  "be considered normal": { meaning: "被认为是正常的", note: "consider 的补语前通常不用 as。" },
  "be serious about": { meaning: "认真对待……；对……当真" },
  "be wise to sth": { meaning: "察觉 / 看穿某事", note: "与 it is wise to do 的“做某事很明智”不同。" },
  ...
  "consider a (to be) b": { meaning: "认为 A 是 B", note: "B 是宾语补足语，通常不用 as。" },
  "consider doing sth": { meaning: "考虑做某事", note: "consider 后接 doing，不接 to do 表示“考虑”。" },
  ...
  "decrease by": { meaning: "减少了……", note: "by 后接变化幅度。" },
  "decrease to": { meaning: "减少到……", note: "to 后接最终数值。" },
  ...
  "it is wise to do": { meaning: "做某事是明智的 / 可取的" },
  ...
  "prosper under stable conditions": { meaning: "在稳定条件下繁荣发展" },
  ...
  "serious consequences": { meaning: "严重后果" },
  "store energy": { meaning: "储存能量" },
  "store grain": { meaning: "储存粮食" },
  "store up supplies": { meaning: "储备物资" },
  "businesses prosper": { meaning: "企业 / 生意兴旺" },
  "at a disadvantage": { meaning: "处于不利地位" },
  "the disadvantage of": { meaning: "……的缺点 / 不利之处" },
  "a wise decision": { meaning: "明智的决定" },
...
// established by earlier passages.
for (const [key, value] of Object.entries(passage4PhraseGuides)) {
  phraseGuides[key] = mergePhrase(phraseGuides[key], value);
}
Object.assign(phraseAliases, passage4PhraseAliases);
...
// established by earlier passages.
for (const [key, value] of Object.entries(passage5PhraseGuides)) {
  phraseGuides[key] = mergePhrase(phraseGuides[key], value);
}
Object.assign(phraseAliases, passage5PhraseAliases);
...
Object.assign(phraseGuides, passage2001P2QuestionPhraseGuides);
Object.assign(phraseAliases, passage2001P2QuestionPhraseAliases);
Object.assign(collocationGlosses, passage2001P2QuestionCollocationGlosses);
...
for (const [key, value] of Object.entries(passage2010P1PhraseGuides)) {
  phraseGuides[key] = mergePhrase(phraseGuides[key], value);
}
Object.assign(phraseAliases, passage2010P1PhraseAliases);
Object.assign(collocationGlosses, passage2010P1CollocationGlosses);
Object.assign(familyGlosses, passage2010P1FamilyGlosses);
...
Object.assign(phraseGuides, passage2000P1QuestionPhraseGuides);
Object.assign(phraseAliases, passage2000P1QuestionPhraseAliases);
Object.assign(collocationGlosses, passage2000P1QuestionCollocationGlosses);
...
for (const [expression, key] of Object.entries(passage2010P3SourcePhraseAliases)) {
  if (!phraseAliases[expression]) {
    const guide = passage2010P3SourcePhraseGuides[key];
    const stable = Object.values(phraseGuides).find(existing => existing.canonical.toLowerCase() === guide.canonical.toLowerCase());
    if (stable) phraseAliases[expression] = stable.key;
    else { phraseGuides[key] = guide; phraseAliases[expression] = key; }
  }
  if (!collocationGlosses[expression]) collocationGlosses[expression] = passage2010P3SourceCollocationGlosses[expression];
}
...
Object.assign(phraseAliases, passage2010P5PhraseAliases);
...
for (const [key, value] of Object.entries(passage2012P2PhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, passage2012P2PhraseAliases);
for (const [key, value] of Object.entries(passage2012P2PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(passage2012P3PhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, passage2012P3PhraseAliases);
for (const [key, value] of Object.entries(passage2012P3PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(passage2012P4PhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, passage2012P4PhraseAliases);
for (const [key, value] of Object.entries(passage2012P4PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(passage2012P5PhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, passage2012P5PhraseAliases);
for (const [key, value] of Object.entries(passage2012P5PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}
...
for (const [key, value] of Object.entries(writing2012APhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, writing2012APhraseAliases);
for (const [key, value] of Object.entries(writing2012APhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(writing2012BPhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, writing2012BPhraseAliases);
for (const [key, value] of Object.entries(writing2012BPhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}
...
for (const [key, value] of Object.entries(passage2011P1PhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, passage2011P1PhraseAliases);
for (const [key, value] of Object.entries(passage2011P1PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(writing2011BPhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, writing2011BPhraseAliases);
for (const [key, value] of Object.entries(writing2011BPhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(writing2011APhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, writing2011APhraseAliases);
for (const [key, value] of Object.entries(writing2011APhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(translation2011PhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, translation2011PhraseAliases);
for (const [expression, key] of Object.entries(translation2011SourcePhraseAliases)) {
  const guide = translation2011SourcePhraseGuides[key] ?? phraseGuides[key];
  if (!phraseAliases[expression]) {
    const existing = Object.values(phraseGuides).find(value => value.canonical.toLowerCase() === guide.canonical.toLowerCase());
    const canonicalKey = existing?.key ?? key;
    phraseGuides[canonicalKey] ??= guide;
    phraseAliases[expression] = canonicalKey;
  }
  collocationGlosses[expression] ??= translation2011SourceCollocationGlosses[expression];
}
for (const [key, value] of Object.entries(translation2011PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}
...
Object.assign(phraseAliases, passage2011P3PhraseAliases);
for (const [key, value] of Object.entries(passage2011P3PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}

for (const [key, value] of Object.entries(passage2011P2PhraseGuides)) {
  if (!phraseGuides[key]) phraseGuides[key] = value;
}
Object.assign(phraseAliases, passage2011P2PhraseAliases);
for (const [key, value] of Object.entries(passage2011P2PhraseGlosses)) {
  if (!collocationGlosses[key]) collocationGlosses[key] = value;
}
...
export function getWordKnowledge(headword: string, context?: { articleId?: string; sentenceId?: string; sourceId?: string }) {
  if (context?.articleId === "2011-translation") {
    const reviewed = getTranslation2011ReviewedKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (reviewed) return reviewed;
  }
  if (context?.articleId === "2012-translation") {
    const reviewed = getTranslation2012ReviewedKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (reviewed) return reviewed;
  }
  ...
  if (context?.articleId === "2001-cloze") {
    const knowledge = getCloze2001SourceKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (knowledge) return knowledge;
  }
  if (context?.articleId === "2001-p2") {
    const knowledge = getPassage2001P2SourceKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (knowledge) return knowledge;
  }
  ...
  if (context?.articleId === "p2") {
    const knowledge = getPassage2000P2WordKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (knowledge) return knowledge;
  }
  if (context?.articleId === "p1") {
    const knowledge = getPassage2000P1WordKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (knowledge) return knowledge;
  }
  ...
  if (context?.articleId === "2011-writing-a") {
    const contextualKnowledge = getWriting2011AWordKnowledge(normalized(headword), context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  if (context?.articleId === "2011-translation") {
    const contextualKnowledge = getTranslation2011WordKnowledge(normalized(headword), context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  ...
  if (context?.articleId === "2011-p3") {
    const contextualKnowledge = getPassage2011P3WordKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  if (context?.articleId === "2011-p2") {
    const contextualKnowledge = getPassage2011P2WordKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  if (context?.articleId === "2011-p1") {
    const contextualKnowledge = getPassage2011P1WordKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  ...
  if (context?.articleId === "2012-p2") {
    const contextualKnowledge = getPassage2012P2WordKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  if (context?.articleId === "2012-p3") {
    const contextualKnowledge = getPassage2012P3WordKnowledge(normalized(headword), context.sourceId ?? context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  if (context?.articleId === "2012-p4") {
    const contextualKnowledge = getPassage2012P4WordKnowledge(normalized(headword), context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  ...
  if (context?.articleId === "2012-writing-a") {
    const contextualKnowledge = getWriting2012AWordKnowledge(normalized(headword), context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }
  if (context?.articleId === "2012-writing-b") {
    const contextualKnowledge = getWriting2012BWordKnowledge(normalized(headword), context.sentenceId);
    if (contextualKnowledge) return contextualKnowledge;
  }

...
账号面板会区分登录服务检查中、请求失败和服务暂不可用。网络异常时可点击「重试检查登录服务」，访客恢复联网或重新打开错误面板也会重查；检查失败会保留当前本机记录归属。仅邮件服务不可用时，已设置密码的账号仍可选择密码登录。实现与验证见 [登录检查修复报告](docs/change-reports/2026-09-18-login-service-check.md)。
